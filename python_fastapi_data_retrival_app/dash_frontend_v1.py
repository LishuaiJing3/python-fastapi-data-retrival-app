...
app.layout = dbc.Container(
    [
        dbc.Row([dbc.Col([html.H2("Download data example", style={"text-align": "center"})])]),
        dcc.DatePickerRange(
...
@app.callback(
    [Output("output-container-date-picker-range", "children"), Output("table", "data"), Output("table", "columns")],
...
def func(n_clicks_btn, download_type, start_date, end_date):
    if n_clicks_btn:
        response = requests.post(
            "http://localhost:8000/data",
            json={"start_date": start_date, "end_date": end_date, "columns": ["HOME_ID", "DEV_ID"]},
        )
        if response.status_code == 200:
            data = response.json()
            df = pd.DataFrame(data)
